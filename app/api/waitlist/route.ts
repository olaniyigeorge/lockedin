...
export async function POST(
  ...
  try {
    const { full_name, email, discovery_location }: WaitlistPayload =
      await req.json();
...
export async function GET(
  req: NextRequest
): Promise<NextResponse<GetWaitlistResponse>> {
  try {
    ...
    const {
      ...
      startDate,
      endDate,
    }: WaitlistFilterPayload = Object.fromEntries(req.nextUrl.searchParams);
