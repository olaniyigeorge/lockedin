...
import { HabitTaskWithEntries, iGetHabitTaskResponse, iHabitTaskEntry, iUpdateHabitTaskResponse, UpdateHabitTaskPayload } from "./docs";
 ...
 */
export async function GET(request: NextRequest): Promise<NextResponse<iGetHabitTaskResponse>> {
    ...
    try {
		...
		const habitTaskDoc = await HabitTask.findById(id)
			.populate({
				path: "owner",
 ...
 */
export async function PATCH(request: NextRequest): Promise<NextResponse<iUpdateHabitTaskResponse>> {
	const url = new URL(request.url);
	const id = url.pathname.split("/").pop();
	...
	try {
		...
		if (habitTask.owner.toString() !== sessionData?.user?.id) {
			return NextResponse.json(
				{ message: "You are not authorised to update this habit task" }, { status: 401 });
		}
		...
		return NextResponse.json({ message: "Successfully updated habit task" }, { status: 200 });
	} catch (error) {
		console.error(error);
		return NextResponse.json(
			{ 
...
try {
    ...
    if (habitTask.owner.toString() !== sessionData?.user?.id) {
    return NextResponse.json({ message: "You are not authorised to delete this habit task" }, { status: 403 });
    }
