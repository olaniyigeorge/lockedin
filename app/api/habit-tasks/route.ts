...
export async function GET(request: Request) {
  try {
    ...
    if (owner) {
      ...
      const filter: { owner: string; accessibility?: string } = {
        owner: owner,
      };
      if (accessibility) {
      ...
      const entries = await HabitTaskEntry.find({
        habit: { $in: habitTaskIds },
      });
    ...
    if (id) {
      ...
      const habit_task = await HabitTask.findById(id).populate({
        path: "owner",
        select: "-password",
        model: User,
      });
      ...
      return new Response(JSON.stringify(habitTaskWithEntries), {
        status: 200,
      });
    }
    ...
    if (accessibility) {
      ...
      const public_habit_tasks = await HabitTask.find({
...
export async function PATCH(request: Request) {
  const { title, description, aspect, accessibility, start_date, end_date } =
    await request.json();
